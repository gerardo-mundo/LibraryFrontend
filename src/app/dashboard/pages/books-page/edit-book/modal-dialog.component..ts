...
export class ModalDialogComponent implements OnInit {
	...
	public form: FormGroup = this.fb.group({
		...
		adquisition: [this.book?.adquisition, [Validators.required, Validators.minLength(4), customValidators.cantBeZero]],
		...
		copies: [this.book?.copies, [Validators.required, customValidators.cantBeZero]],
	});
	...
	saveBook() {
		...
		this.bookService.updateBook(this.book.id!, this.form.value).subscribe({
			next: () => {
				...
				this.bookService.getBooks().subscribe((books) => this.updatedBooks.emit(books));
			},
