...
export class RegisterPageComponent {
	...
	public selectedOption(event: SelectButtonOptionClickEvent) {
		...
		if (this.isStudent) {
			this.registerForm
				.get('enrollmentNum')
				?.setValidators([Validators.required, Validators.maxLength(8), Validators.minLength(8)]);
		} else {
			null;
		}
		...
		if (this.isEmployee) {
			this.registerForm
				.get('employeeKey')
				?.setValidators([Validators.required, Validators.maxLength(10), Validators.minLength(8)]);
		} else {
			null;
		}
